...
export default function Home(props: any) {
  ...
  const router = useRouter();  // Next.js의 useRouter hook을 사용
    ...
    async function fetchData() {
      try {
        ...
        if (props.params.userId) {
          const postResponse = await axios.post('/api/getPosts/', {
            userId: props.params.userId,
          });
          setPosts(postResponse.data);
          setPostFeed(<MyPostFeed initialPosts={postResponse.data} session={props.params.userId} />);
        }
...
useEffect(() => {
  async function fetchFollowCounts() {
    try {
      const response = await axios.post('/api/followCount', {
        userId: props.params.userId // 현재 로그인한 사용자의 ID
      });
  ...
  // 만약 userId가 설정되어 있다면 팔로워 및 팔로잉 수를 가져옵니다.
  if (props.params.userId) {
    fetchFollowCounts();
